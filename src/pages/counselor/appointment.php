...
$search_student= "SELECT student_id FROM student_account WHERE (institution_code = '$institution_code')";

try {
    $student_stmt=$pdo->prepare($search_student);
    $student_stmt->execute();
    $results=$student_stmt->fetchAll();
}catch(Exception $ex) {
    echo($ex -> getMessage());
}
?>
                            ...
                            <div class="container-fluid">
                                <form action="./php/save_schedule.php" method="post" id="schedule-form">
                                    ...
                                    <div class="form-group mb-2">
                                        ...
                                        <select class="selectpicker form-control form-control-sm" data-live-search="true" name="student_id" id="student_id" placeholder="Search Student ID" required>
                                            <option></option>
                                            <?php foreach($results as $output) {?>
                                            <option value="<?php echo $output["student_id"];?>"><?php echo $output["student_id"];?></option>
                                            <?php }?>
                                        </select>
                                    </div>
                                    <div class="form-group mb-2">
...
<script src="./js/scriptcon.js"></script>